...
typedef struct CobaltExtensionPlatformServicePrivate {
  ...
  virtual void* Send(const void* data, uint64_t data_length, uint64_t* output_length, bool* invalid_state) = 0;
...
struct ContentEntitlementCobaltExtensionPlatformService : public CobaltExtensionPlatformServicePrivate {
  ...
  void* Send(const void* data, uint64_t length, uint64_t* output_length, bool* invalid_state) override {
    bool result = false;
