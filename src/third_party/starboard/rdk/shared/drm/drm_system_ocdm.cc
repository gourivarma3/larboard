...
void Session::GenerateChallenge(std::string_view type,
  ...
  if (opencdm_construct_session(
          ...
          initialization_data.size(), nullptr, 0, &session_callbacks_, this,
          &session) != ERROR_NONE ||
      !session) {
...
void Session::Update(std::string_view key, int ticket) {
  ...
  if (opencdm_session_update(session_.get(), reinterpret_cast<const uint8_t*>(key.data()),
                             key.size()) != ERROR_NONE) {
...
void DrmSystemOcdm::UpdateServerCertificate(int ticket,
  ...
  auto status = opencdm_system_set_server_certificate(
      ocdm_system_, reinterpret_cast<const uint8_t*>(certificate.data()), certificate.size());
...
SbDrmSystemPrivate::DecryptStatus DrmSystemOcdm::Decrypt(::starboard::InputBuffer* buffer) {
  SB_NOTREACHED();
  return kFailure;
}
